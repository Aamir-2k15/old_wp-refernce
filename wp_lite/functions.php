...
function loop_posts($atts) {
	...
	$order =  $atts['order'];
	$n = 1;
	$related_args = array(	'post_type' => $pt,'category_name' => $cate,'posts_per_page' => $per_page,    'orderby'        => 'ID',    'order' => $order);
...
$output .='<div class="post-'.get_the_ID().' '.$pt.' '.$pt.'-'.$n.'">'; 
 if (has_post_thumbnail()) { 
 $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' ); 
    $output .='<div class="loop-'.$pt.'-img-container"><a href="'.get_the_permalink(get_the_ID()).'"><img src="'.$image[0].'" class="img-fluid ml-auto mr-auto"/></a></div>';
     } 
    $output .='<h4 class="loop-'.$pt.'-title"><a href="'.get_the_permalink(get_the_ID()).'">'.get_the_title().'</a></h4>';
    	if($excerpt != "") {$output .='<p class="excerpt '.$pt.'-excerpt">'.  get_the_excerpt() .'</p>';}
...
//EXAMPLE: [loop_pt pt='posts' cate='popular' order='ASC' col_width='4' per_page='8' excerpt='yes' readmore_title='readmore']
